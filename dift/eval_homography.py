...
if __name__ == "__main__":
    ...
    parser.add_argument('--feat', choices=['dift_sd', 'dift_adm', 'dift_vggt', 'dift_dino', 'dift_croco'], default='dift_sd', help="which feature to use")
    ...
    for seq_idx, seq_name in tqdm(enumerate(seq_names)):
        ...
        for im_idx in range(2, 7):
            ...
            if args.mode == 'ransac':
                ...
                if len(pts_a) >= 4 and len(pts_b) >= 4:
                    H, mask = cv2.findHomography(pts_a, pts_b, cv2.RANSAC, ransacReprojThreshold=3)
                else:
                    print(f"Skipping due to too few matches: {len(pts_a)} points")
