...
def main(cfg):
    ...
    if cfg.global_feature and cfg.local_feature:
        up_conditioning_channels = 2048 
    elif cfg.local_feature:
        up_conditioning_channels = 1024
    elif cfg.global_feature:
        up_conditioning_channels = 1024
    else: 
        up_conditioning_channels = 1024
    ...
    pose_guider_two = PoseGuider_Up(
        conditioning_channels = cond_channels,
        conditioning_embedding_channels=320,
        up_conditioning_channels=up_conditioning_channels,
    ...
    if cfg.local_data:
        if not cfg.inference:
            if cfg.train_vggt:
                co3d_directory = "mnt/data2/minseop/co3d_wds"                
                realestate_directory = "/mnt/data2/minseop/co3d_wds"           
            else:
            ...
            realestate_directory = "/mnt/data2/minseop/co3d_wds_val"        
    ...
    if accelerator.is_main_process:
        ...
        accelerator.init_trackers(
            entity_name="nvs-vggt"
            project_name="minkyung",
            ...
            with accelerator.accumulate(net):
                ...
                with torch.no_grad():
                    ...
                    with torch.cuda.amp.autocast(dtype=dtype):
                        ...
                        mid_feats = torch.stack(mid_feats, dim=0) # (B, 4, View_num, 37 x 37, 2048)
                        ...
                        if cfg.global_feature and cfg.local_feature:
                            ...
                            mid_feats = mid_feats[:,:,:,:,1024:] # local attention output
                            feat_dim = 1024
                        elif cfg.global_feature:
                            mid_feats = mid_feats[:,:,:,:,:1024] # global attention output 
                            feat_dim = 1024
                        ...
                        # Select features from cfg.layers that exist in layer
                        selected_feats = [layer[i] for i in cfg.layers if i in layer]

                        # Concatenate along dim=1 (the "layer" axis)
                        mid_feats = torch.stack(selected_feats, dim=1)  # (B, N_selected, View_num, 37 x 37, 2048 or 1024)
                        
                        N_selected = mid_feats.shape[1]
                        # Which layer to use?
                        if cfg.use_pca:
                            mid_feats = mid_feats.reshape(batch_size * N_selected, V, 37, 37, -1)
                            ...
                            for i, feat in enumerate(mid_feats):
                                reduced_tensor, _, _ = compute_pca(feat, feat_dim) # 9
                                featstack.append(reduced_tensor)
                            
                            featstack = torch.stack(featstack, dim=0).reshape(batch_size, N_selected, V, 37 * 37, feat_dim)
                            mid_feats = featstack                                    
                ...
                if not cfg.inference:
                    results_dict = net(
                        ...
                        geo_attn_proc_hooker=geo_attn_proc_hooker if cfg.geo_second else None,
...
if __name__ == "__main__":
    ...
    parser.add_argument("--config", type=str, default="././train_configs/train_co3d_unified_vggt_mk.yaml")
