...
def postprocess_fn(sample):
    ...
    try:
        ...
        for single_frame_data in frame_data[:per_batch*3]:
            ...
            # pose = np.array(split_data[7:19]).reshape(3,4)
...
# def preprocess(image_batches, batch, model, instance, k, args):
#     src_images = []
#     image_tensors = []
...
#         instance_savedir = os.path.join("/mnt/tmp/realestate", instance + "_" + str(k))

#         save_dataset(instance_savedir, instance, image_tensors, focals, poses, pts3d)
...
def main(args):
    ...
    urls = [f's3://generation-research/realestate/realestate_{num:06}.tar' for num in num_list]
    ...
    for data in dataset:
        ...
        #     ult = [image_batches, batch, model, instance, k, args]
        #     ult_list.append(ult)

        # with multiprocessing.Pool(processes=args.num_processes) as pool:
        #     for result in pool.starmap(preprocess, ult_list):
        #         try:
        #             suc, instance = result
        #             print(suc)
        #         except:
        #             instance = result
        #             print(f"Frame {instance} not saved.")
