...
def main(cfg):
    ...
    if cfg.local_data:
        if not cfg.inference:
            ...
            realestate_directory = "/mnt/data2/minseop/co3d_val_wds"        
    ...
    # postprocess_fn_1 = partial(postprocess_realestate, num_viewpoints=num_ref_viewpoints, interpolate_only = cfg.interpolate_only)
    # train_dataset = (
    ...
    # postprocess_fn_2 = partial(postprocess_co3d, num_viewpoints=num_ref_viewpoints)
    # train_dataset_2 = (
            ...
            with accelerator.accumulate(net):
                ...
                if not cfg.inference_from_data:
                    ...
                    # if cfg.dataset.num_ref != 1: 
                        src_idx = random.sample(range(sampling_views - 6), 1)[0]
                        src_idx = random.sample(range(src_idx, src_idx + 5), cfg.dataset.num_ref)
                ...
                for target_idx in target_idx_list:
                    ...
                    with torch.no_grad():
                        ...
                        if not cfg.inference or cfg.inference_from_data:
                            if not cfg.train_vggt:
                                ...
                                # torch.matmul(extrinsic[:,target_idx][:,None,None,None,:3,:3], (pointmaps - w2c[:,target_idx][:,None,None,None,:3,3]).unsqueeze(-1))             
                        ...
                        # import pdb; pdb.set_trace()
                        # save_image(pointmaps[:,target_idx].permute(0,3,1,2)[:,2:], "pts.png")
                    ...
                    if not cfg.inference:
                        ...
                        with torch.no_grad():
                            ...
                            depth = apply_heatmap(1 / depth_map[:,:1]).to(device)        
                            normal = convert_depth_to_normal(depth_map[:,:1])

                            space = torch.ones(1,3,512,80).to(device)
                            
                            gt_depth = apply_heatmap(1 / (tgt_depth_norm[:,:1]*0.5+0.5)).to(device)  
                            ...
                            # tgt_dict["depth"].append(depth)
                            # tgt_dict["image"].append(results_dict['img_pred'])
                            # tgt_dict["normal"].append(normal * 0.5 + 0.5)
                            # tgt_dict["point"].append((normal * 0.5 + 0.5)[:,2:])
                            # tgt_dict["warped"].append(zero_mask * (warped_image * 0.5 + 0.5))
                            ...
                            # except:
                            #     stack_img = torch.cat((images["tgt"].to(device), space, warped_image * 0.5 + 0.5, space, tgt_pointmap*0.5+0.5, space, tgt_depth_norm*0.5+0.5, space, results_dict['img_pred'], space, results_dict['geo_pred'][:,2:].repeat(1,3,1,1)), dim=-1).detach()[0]  
                            
                            # gen_img = torch.cat((images["tgt"].to(device), space, results_dict['img_pred'], space, diff_gt), dim=-1).detach()[0]
                            ...
                            if not cfg.inference_from_data:
                                ...
                                dir = f"/mnt/data2/minseop/val_results/{now}"
                                ...
                                space = torch.ones(1,3,512,80).to(device)
                                large_space = torch.ones(1,3,512,130).to(device)
                                ...
                                for i, ref in enumerate(ref_images):
                                    ...
                                    if i != ref_images.shape[0]-1:
                                        ref_stack.append(horz_space[0])
                                    
                                ref_images = torch.cat(ref_stack, dim=-2).unsqueeze(0)
                                ...
                                # gt_points = torch.cat((correspondence['tgt'], images['tgt'].permute(0,2,3,1)), dim=-1).reshape(B,-1,6)
                                
                                pointmap_list.append(points)
                                
                                # import pdb; pdb.set_trace()
                                
                                large_space = torch.ones(1,3,512,512).to(device)
                                
                                # val_img = torch.cat((images["tgt"].to(device), space, zero_mask * (warped_image * 0.5 + 0.5), space, tgt_pointmap*0.5+0.5, space, gt_depth, space, results_dict['img_pred'], space, results_dict['geo_pred'], space, depth, space, normal * 0.5 + 0.5), dim=-1).detach()[0] 
                                
                                # gt_depth
                                
                                gt_normal = convert_depth_to_normal(gt_depth[:,:1])
                                val_img = torch.cat((warped_image * 0.5 + 0.5, space, results_dict['img_pred'], space, depth, space, normal * 0.5 + 0.5, space, space, large_space, large_space, space, images["tgt"].to(device), space, gt_depth, space, gt_normal *0.5 + 0.5), dim=-1).detach()[0] 
                                
                                # import pdb; pdb.set_trace()
                                
                                save_image(val_img, f"{dir}/target_view_{target_idx}.png")
                                save_image(stack_img, f"{dir}/target_stk_{target_idx}.png")
                                
                                if target_idx == target_idx_list[-1]:
                                    
                                    pointmap_stack_locs = torch.stack(pointmap_list, dim=0).to(device)[...,:3]
                                    
                                    ext = batch['points'].permute(0,1,3,4,2)
                                    
                                    rotated_pts = []
                                    
                                    for i, idx in enumerate(target_idx_list):
                                        pts_map = torch.matmul(extrinsic[:,idx][:,None,:3,:3], (pointmap_stack_locs[i] - w2c[:,idx][:,None,:3,3]).unsqueeze(-1))             
                                        new_pts = torch.matmul(w2c[:,target_idx][:,None,:3,:3], pts_map).squeeze(-1) + w2c[:,target_idx][:,None,:3,3]
                                        
                                        rotated_pts.append(new_pts)
                                        
                                    all_pts = torch.cat(rotated_pts, dim=0)
                                    all_rgb = torch.cat(pointmap_list, dim=0).to(device)[...,3:]
                                
                                    every = torch.cat((all_pts, all_rgb), dim=-1)
                                    
                                    torch.save(every, f"{dir}/all_pts.pt")
                                    
                                    camera_dict = {
                                        "ref_ext": ref_camera['pose'][0],
                                        "ref_focal": ref_camera['focals'][0],
                                        "tgt_ext": extrinsic[:,target_idx_list][0],
                                    ...
                                    #     import pdb; pdb.set_trace()
                                    #     json.dump(tgt_camera, f, indent=4)
                                    ...
                                    # (ref_pointmaps - ptsmap_min[None,...,None,None]) / (ptsmap_max[None,...,None,None] - ptsmap_min[None,...,None,None]) 
                                
                                #     save_image(stack_img, f"{dir}/stack_target_view_{target_idx}.png")
                                    
                                    # for batch_idx in range(B):
                                    #     pointmap_vis(every.reshape(-1,6), dir, ref_camera['pose'][batch_idx], ref_camera['focals'][batch_idx], num_cams=2)
                                
                                #     import pdb; pdb.set_trace()
                                
                                # # Warped image / Predicted depth / Predicted normal / Predicted pts 1 / Predicted pts 2 / 
                                    
                                # val_img = torch.cat((zero_mask * (warped_image * 0.5 + 0.5), space, tgt_pointmap*0.5+0.5, space, gt_depth, space, results_dict['img_pred'], space, results_dict['geo_pred'], space, depth, space, normal * 0.5 + 0.5), dim=-1).detach()[0]  
