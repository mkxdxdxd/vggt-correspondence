...
class Scannetdataset(torch.utils.data.Dataset):
    ...
    def _get_consecutive_two_frame_interval(self, scene_id, img_path):
        assert self.num_views == 2 and self.interval is not None
        ...
        if idx_base + num_views + (interval-1)  < num_kfs:
            idx_list = np.array([idx_base, idx_base + num_views + interval -2])
            img_paths = [scene_kfs[item] for item in idx_list]
        else:
            num_back = num_kfs - idx_base - 1
            num_front = num_views + (interval-1) - 1 - num_back
            idx_start = idx_base - num_front
            if idx_start < 0:
                idx_start = 0
                idx_list = np.arange(len(scene_kfs))
                ...
                idx_list = np.array([idx_start, idx_start + num_views + interval -2])
                img_paths = [scene_kfs[item] for item in idx_list]
    ...
    def load_seq(self, image_paths, resolution):
        ...
        images_clip = np.stack([self.clip_image_processor(
            images=Image.open(path), return_tensors="pt"
        ).pixel_values[0] for path in image_paths])
        ...
        depths = [np.array(Image.open(p)).astype("float")/1000. for p in depth_path]
        ...
        return images, depths, depth_inv_norm_full, poses, k, prompts, images_clip
    ...
    def __getitem__(self, idx):
        ...
        if self.data_load_mode=='fix_interval':
            ...
            p_rand=0 #random.random() # modified
        ...
        # newly added
        elif self.data_load_mode=='two_frame_interval':
            image_paths=self._get_consecutive_two_frame_interval(
                scene_id, img_path)
            mask=np.ones(len(image_paths))
            
        elif self.data_load_mode=='two_stage':
            image_paths, mask=self._get_consecutive_kfs_inp(
                scene_id, img_path)

        images, depths, depth_inv_norm, poses, K, prompts, images_clip = self.load_seq(
        ...
        images_clip = images_clip.astype(np.float32)
