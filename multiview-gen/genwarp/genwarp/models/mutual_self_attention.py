        ...
        def hacked_basic_transformer_inner_forward(
            ...
            if self.only_cross_attention:
                ...
                if mode == "write":
                    ...
                    self.bank[reference_idx] = []
                    self.bank[reference_idx].append(norm_hidden_states.clone())

                    if self.bank_unnorm:
                        self.bank_unnorm[reference_idx] = []
                        self.bank_unnorm[reference_idx].append(hidden_states.clone())
                ...
                if mode == "read":
                    ...
                    bank_fea = [
                        rearrange(
                            self.bank[ref_idx][0].unsqueeze(1).repeat(1, video_length, 1, 1),
                            "b t l c -> (b t) l c",
                        )
                        for ref_idx in self.bank.keys()
                    ]
                    ...
                    if self.bank_unnorm:
                        bank_fea_unnorm = [
                            rearrange(
                                self.bank_unnorm[ref_idx][0].unsqueeze(1).repeat(1, video_length, 1, 1),
                                "b t l c -> (b t) l c",
                            )
                            for ref_idx in self.bank_unnorm.keys()
                        ]
                    ...
                    # import pdb; pdb.set_trace()
                    # attn_dic = {"return_attention_map" : True}
    ...
    def update(self, writer, correspondence=None, dtype=torch.float16):
        ...
        if self.reference_attn:
            ...
            for r, w in zip(reader_attn_modules, writer_attn_modules):
                for key in w.bank.keys():
                    r.bank[key] = [v.clone().to(dtype) for v in w.bank[key]]

                    if self.bank_unnorm:
                        r.bank_unnorm[key] = [v.clone().to(dtype) for v in w.bank_unnorm[key]]
