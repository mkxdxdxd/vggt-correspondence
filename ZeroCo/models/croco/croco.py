...
class CroCoNet(nn.Module):
    ...
    def forward(self, img_target, img_source, output_correlation='ca_map'):
        ...
        if output_correlation == 'enc_feat':
            ...
            feat1 = l2norm(feat1.permute(0,2,1)).permute(0,2,1)    # torch.Size([1, 196, 1024])
            feat2 = l2norm(feat2.permute(0,2,1)).permute(0,2,1)
            refined_corr = torch.einsum('bnd, bmd -> bnm', feat1, feat2)   # torch.Size([1, 196, 196])
            ...
            camap1 = [attn.mean(dim=1) for attn in camap1] # 12
            ...
            camap1 = torch.stack(camap1, dim=1) # [1, 12, 196, 196]
            camap2 = torch.stack(camap2, dim=1) # [1, 12, 196, 196]
        ...
        return flow_est # torch.Size([1, 2, 224, 224])
