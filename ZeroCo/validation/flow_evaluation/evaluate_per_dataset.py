    ...
    for i_batch, mini_batch in pbar:
        ...
        if args.model == 'croco':
            ...
            if args.dense_zoom_in:
                ...
                flow_est = network(target_img, source_img, output_correlation=output_correlation) # b 2 224 224
        ...
        # eval_img_size -> resize input img to 224 -> model's output_flow 224 -> 
            ...
            H_518, W_518 = args.model_img_size
            source_img = F.interpolate(source_img, size=(H_518, W_518), mode='bilinear', align_corners=False)   # b 3 224 224
            target_img = F.interpolate(target_img, size=(H_518, W_518), mode='bilinear', align_corners=False)

            img = torch.cat([source_img, target_img], dim=0).to(device)
            flow_est = network(images=img, layer=4) # b 1024 37 37
            if args.uncertainty:
                flow_est = flow_est['flow_estimates'][0]
                
            flow_est = F.interpolate(flow_est, size=(H_orig, W_orig), mode='bilinear', align_corners=False)  
            # corr_size = H_224//16
            flow_est[:,0,:,:] *= W_orig/518
            flow_est[:,1,:,:] *= H_orig/518
